...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train structural-HO augmented joint model with pair BCE + HO InfoNCE. "
            "Train message passing uses HO_train-derived relations; val/test use no HO edges."
        )
    )
    parser.add_argument(
        "--kg-edges",
    ...
    parser.add_argument(
        "--disable-struct-ho",
        action="store_true",
        help="Disable structural HO graph augmentation (kept for ablation/debug).",
    )
...
def train_one_epoch(
    ...
    model.train()
    train_graph = bundle.graph_for_split("train")
    edge_index = train_graph.edge_index.to(device)
    edge_type = train_graph.edge_type.to(device)
...
def evaluate(
    ...
    model.eval()
    graph = bundle.graph_for_split(split_name)
...
def summarize_counts(bundle: StructHODataBundle) -> Dict[str, object]:
    pair_counts = {}
    ...
    for split_name in ("train", "val", "test"):
        graph = bundle.graph_for_split(split_name)
        graph_counts[split_name] = {
            "num_nodes": graph.num_nodes,
            "num_edges": int(graph.edge_index.size(1)),
            "num_relations": graph.num_relations,
            "num_nodes_by_type": graph.num_nodes_by_type,
        }
    ...
    return {
        ...
        "pairs": pair_counts,
        "ho": ho_counts,
        "ho_struct_edge_counts": bundle.ho_struct_edge_counts,
    }


def print_struct_ho_counts(bundle: StructHODataBundle) -> None:
    for split_name in ("train", "val", "test"):
        counts = bundle.ho_struct_edge_counts[split_name]
        payload = {"split": split_name}
        payload.update({rel: int(counts.get(rel, 0)) for rel in STRUCT_HO_RELATIONS})
...
def main() -> None:
    ...
    bundle = prepare_struct_ho_data(
        ...
        enable_struct_ho=(not args.disable_struct_ho),
    )
    print_struct_ho_counts(bundle)
    ...
    summary = {
        ...
        "struct_ho_enabled": not args.disable_struct_ho,
        "lambda_ho": args.lambda_ho,
        "tau": args.tau,
